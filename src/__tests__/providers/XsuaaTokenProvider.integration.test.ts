...
describe('XsuaaTokenProvider Integration', () => {
  ...
  describe('Service key to session conversion', () => {
    it('should convert XSUAA service key to session', async () => {
      ...
      // Load service key
      const serviceKey = await serviceKeyStore.getServiceKey(xsuaaConfig.btp_destination);
      if (!serviceKey) {
        throw new Error(`Service key not found for destination "${xsuaaConfig.btp_destination}" in directory "${serviceKeysDir}". Please ensure the service key file exists.`);
      }
      ...
      const savedSession = await sessionStore.loadSession(xsuaaConfig.btp_destination);
    ...
    it('should validate token', async () => {
      if (!hasRealXsuaaConfig) {
        console.warn('⚠️  Skipping XSUAA token validation test - no real config');
        return;
      }

      if (!xsuaaConfig.btp_destination || !xsuaaConfig.mcp_url || !serviceKeysDir) {
        console.warn('⚠️  Skipping XSUAA token validation test - missing required config');
        return;
      }
      ...
      if (!serviceKey || !serviceKey.uaaUrl || !serviceKey.uaaClientId || !serviceKey.uaaClientSecret) {
        throw new Error(`Service key not found or incomplete for destination "${xsuaaConfig.btp_destination}" in directory "${serviceKeysDir}". Please ensure the service key file exists and contains valid UAA credentials.`);
      }
