...
- A SAML refusal test asserts `check` and a message fragment only its rule produces; no two rules under one `check` share a message. The README's "Refusal messages" table lists every message the shipped validators produce — update it with the code.
