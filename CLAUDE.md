...
- **Everything is read from the signed element.** Every signature must verify and envelope its target; every SAML `Assertion`/`EncryptedAssertion` must be the signed one or inside it; IDs must be unique. Absent fields are refused, not skipped.
