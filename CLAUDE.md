...
**Live XSUAA checks** (`tests/xsuaa/`, `npm run test:xsuaa`, not in CI) create an XSUAA instance, an `apiaccess` instance and a SAML trust in a real BTP subaccount, run `src/__tests__/integration/xsuaa/`, and remove everything — also on failure. The scripts refuse unless `cf target` matches `XSUAA_CF_API`, `XSUAA_CF_ORG` and `XSUAA_CF_SPACE` exactly — a developer's `cf` may point at a production org — and delete only what `setup.sh` recorded in `.local/owned`; a name that exists unrecorded is refused. A failed teardown stops, keeps `.local/`, and the run exits non-zero. The test IdP's key is generated per run into the gitignored `tests/xsuaa/.local/` — unlike the stand's fixtures, this key is trusted by a real subaccount, so it must never be committed. `btp create security/trust` only trusts IAS tenants; a custom SAML IdP goes through the deprecated `apiaccess` plan's `/sap/rest/identity-providers`.
