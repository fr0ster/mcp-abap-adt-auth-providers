...
```ts
manualPasteStrategy(opts?)                      // prints the URL, reads a pasted code
externalCodeStrategy({ redirectUri, provide })  // provide(authorizationUrl) → payload

// manualPasteStrategy opts: { redirectUri = 'http://localhost:61001/callback', read? }
```

`manualPasteStrategy` still needs a redirect URI: the authorization URL must
carry one, and the exchange must repeat it — the browser simply lands on a dead
localhost and the user copies the code out of the address bar. Its default
matches the browser strategy's port so that one registration at the IdP serves
both. `read` overrides where the pasted value comes from, for a consumer that
has its own prompt.
...
to the baked-in one. This is caught cheaply: before the exchange, compare the
`redirect_uri` inside the pre-built URL with `outcome.redirectUri` and fail with
an explanation, rather than with an opaque `invalid_grant` from UAA.
...
`CallbackServerFactory`; `manualPasteStrategy` under an intercepted
`process.stdout.write` (not one byte to stdout); `externalCodeStrategy`
receiving a URL with `code_challenge` in it.
...
strategy is" table, a separate note that `authorizationCodeProvider` and
`assertionFlow` are replaced by `externalCodeStrategy`, and that the default
port changed. `docs/REFACTORING_PROPOSAL.md` is checked for currency — it
