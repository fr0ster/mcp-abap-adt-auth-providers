...
- **Scope a consumer by what it ships, not by `src/`.** Twice in this arc the breakage lived outside the directory I grepped: the proxy hid it behind an `any`, and `auth-broker` — planned as "tests only" — turned out to construct providers in two *published* `bin/` CLIs, one of which force-cast its config past the type checker so the build stayed green while the flags silently stopped working. Before sizing any consumer task, check `package.json`'s `bin` and `files` entries, and search for `as unknown as` and `: any` around the types being changed: those are where a breaking change hides instead of failing.
